-- 001_initial: the tables the Logos plugin owns, plus its migration ledger.
--
-- Every statement is `IF NOT EXISTS`. Before 0.2.0 these tables were created
-- lazily by whichever tool ran first, with exactly the DDL below, so a database
-- that already has them is adopted as revision 1 without a single change —
-- rows, checkpoints and staged chunks included.
--
-- Applied by `logos.db.migrate.upgrade` inside one transaction, together with
-- this revision's ledger row. Never edit this file once released: its checksum
-- is recorded, and a changed checksum stops every later upgrade.
...
-- Migration ledger: one row per applied revision of this directory.
CREATE TABLE IF NOT EXISTS logos_schema_migrations (
